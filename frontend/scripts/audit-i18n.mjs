...
const FRONTEND_DIRS = args.includeSrc ? [frontendRoot, frontendSrcRoot] : [frontendRoot];
...
function extractKeysFromFile(filePath) {
	...
	const useTranslationSingle = src.match(/useTranslation\(\s*(['"])([^'"]+)\1\s*\)/);
	...
	if (useTranslationArray?.[1]) {
		const nsMatches = [...useTranslationArray[1].matchAll(/(['"])([^'"]+)\1/g)].map((m) => m[2]);
...
function auditBaseDir(baseDir) {
	...
	return { baseDir, langs, missing, parseErrors, totalKeys: uniq.size, localesRoot };
}
	...
	for (const r of reports) {
		...
		if (r.parseErrors.length) {
			...
			for (const e of r.parseErrors) {
				console.log(`- ${path.relative(repoRoot, e.file)} (${e.lng}/${e.ns}): ${e.error}`);
			}
		}
