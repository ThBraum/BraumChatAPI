...
export default function LoginPage() {
  ...
  const stats = [
    ...
    {
      value: "6",
      label: t("auth:hero.stats.languages"),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email?.trim() || !password) {
      ...
      if (!email?.trim() && !password) {
        setError(
          t("auth:login.requiredFields") ?? "Email and password are required",
        );
        setEmailError(t("auth:login.emailRequired") ?? "Email required");
        setPasswordError(
          t("auth:login.passwordRequired") ?? "Password required",
        );
        return;
      }
      if (!email?.trim()) {
    ...
    if (!parsed.success) {
      ...
      if (issues.email && issues.email.length)
        setEmailError(t("auth:login.invalidEmail") ?? issues.email[0]);
      if (issues.password && issues.password.length)
        setPasswordError(t("auth:login.passwordMin") ?? issues.password[0]);
      setError(
        t("auth:login.fixFields") ??
    ...
    try {
      ...
      setError(
        (err as Error)?.message ?? t("auth:login.error") ?? "Login failed",
      );
    } finally {
      setIsSubmitting(false);
    }
            ...
            <div className="grid gap-6 rounded-3xl border border-border/60 bg-card/40 p-6 backdrop-blur-sm sm:grid-cols-2">
              <div className="space-y-3 min-w-0">
                <h2 className="text-xl font-semibold">
                  {t("auth:hero.aboutTitle")}
                </h2>
                <p className="text-sm text-muted-foreground">
                  {t("auth:hero.aboutBody")}
                </p>
                ...
                {stats.map((stat) => {
                  ...
                  return (
                    <div key={stat.label} className={cardClasses}>
                      ...
                      {stat.detail && (
                        <p className="mt-2 text-xs text-muted-foreground/80 whitespace-normal text-left">
                          {stat.detail}
                        </p>
                      )}
                    </div>
              ...
              <CardContent>
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      {t("auth:login.email")}
                      <span className="text-rose-500 font-extrabold drop-shadow ml-1">
                        *
                      </span>
                    </Label>
                    <Input
