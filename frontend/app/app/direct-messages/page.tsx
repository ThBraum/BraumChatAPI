...
export default function DirectMessagesPage() {
    ...
    const tab = ((searchParams.get("tab") as TabKey | null) ?? "invite") as TabKey;
    ...
    const usersQuery = useQuery<UserPublic[]>({
        ...
        queryFn: async () => {
            ...
            return apiFetch(`/users/search?q=${encodeURIComponent(q)}&limit=${pageSize}`);
        },
    ...
    const friendsQueryResult = useInfiniteQuery<UserPublic[]>({
        ...
        initialPageParam: 0,
        queryFn: ({ pageParam }) => {
            const qs = friendsQuery.trim() ? `&q=${encodeURIComponent(friendsQuery.trim())}` : "";
    ...
    const recentThreadsQuery = useInfiniteQuery<Thread[]>({
        ...
        initialPageParam: 0,
        queryFn: ({ pageParam }) => {
            const qs = recentQuery.trim() ? `&q=${encodeURIComponent(recentQuery.trim())}` : "";
            const ws = activeWorkspaceId ? `workspace_id=${encodeURIComponent(activeWorkspaceId)}` : "";
            const wsPart = ws ? `${ws}&` : "";
            return apiFetch(`/dm/threads?${wsPart}limit=${pageSize}&offset=${Number(pageParam ?? 0)}${qs}`);
        },
        getNextPageParam: (lastPage, allPages) => {
            if (!lastPage || lastPage.length < pageSize) return undefined;
            return allPages.length * pageSize;
    ...
    const sendInviteMutation = useMutation({
        ...
        onSuccess: async () => {
            await queryClient.invalidateQueries({ queryKey: queryKeys.friendRequestsOutgoing });
            await queryClient.invalidateQueries({ queryKey: queryKeys.friendRequestsIncoming });
        },
    });
    ...
    const openDmMutation = useMutation({
        mutationFn: async (userId: string) => {
            if (!activeWorkspaceId) throw new Error(t("navigation:directMessagesPage.errors.selectWorkspace"));
    ...
    const incoming = useMemo(() => (incomingQuery.data?.pages ?? []).flat(), [incomingQuery.data]);
    const outgoing = useMemo(() => (outgoingQuery.data?.pages ?? []).flat(), [outgoingQuery.data]);
    const friends = useMemo(() => (friendsQueryResult.data?.pages ?? []).flat(), [friendsQueryResult.data]);
    const recentThreads = useMemo(() => (recentThreadsQuery.data?.pages ?? []).flat(), [recentThreadsQuery.data]);
    ...
    return (
        <div className="p-6 space-y-4">
            <div className="flex items-center justify-between gap-3">
                <h1 className="text-xl font-semibold">{t("navigation:directMessagesPage.title")}</h1>
            </div>

            <div className="flex flex-wrap gap-2">
                <Link href="/app/direct-messages?tab=invite">
                    <Button variant={tab === "invite" ? "secondary" : "outline"} size="sm">
                        {t("navigation:directMessagesPage.tabs.invite")}
                    </Button>
                </Link>
                <Link href="/app/direct-messages?tab=friends">
                    <Button variant={tab === "friends" ? "secondary" : "outline"} size="sm">
                        {t("navigation:directMessagesPage.tabs.friends")}
                    </Button>
                </Link>
                <Link href="/app/direct-messages?tab=recent">
                    <Button variant={tab === "recent" ? "secondary" : "outline"} size="sm">
            ...
            {tab === "invite" && (
                <div className="space-y-6">
                    <Card className="max-w-2xl">
                        <CardHeader>
                            <CardTitle className="text-base">{t("navigation:directMessagesPage.invite.findTitle")}</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-3">
                            <Input
                                placeholder={t("navigation:directMessagesPage.invite.searchPlaceholder")}
                                value={userSearch}
                                                ...
                                                {incomingReq ? (
                                                    <div className="flex gap-2">
                                                        ...
                                                        <Button
                                                            size="sm"
                                                            variant="ghost"
                                                            onClick={() => declineMutation.mutate(incomingReq.id)}
                                                    ...
                                                    <Button
                                                        size="sm"
                                                        onClick={() => sendInviteMutation.mutate(u.display_name)}
                                                        disabled={!canInvite || sendInviteMutation.isPending}
                                                        variant={outgoingReq ? "outline" : "default"}
                                                    >
                                                        {outgoingReq
                                                            ? t("navigation:directMessagesPage.invite.sent")
                                                            : t("navigation:directMessagesPage.invite.invite")}
                                                    </Button>
                                                )}
                                            </li>
                                        );
                            ...
                            {(usersQuery.isError || sendInviteMutation.isError) && (
                                <p className="text-sm text-red-600">
                                    {((usersQuery.error ?? sendInviteMutation.error) as Error)?.message}
                                </p>
                            )}
                        </CardContent>
                    </Card>
                            ...
                            {incoming.map((req) => (
                                <li key={req.id} className="flex items-center justify-between rounded-md border px-3 py-2">
                                    <div className="min-w-0">
                                        ...
                                        <div className="truncate text-xs text-muted-foreground">status: {req.status}</div>
                                    </div>
                                    <div className="flex gap-2">
                                        <Button
                                            size="sm"
            ...
            {tab === "friends" && (
                <div className="space-y-4">
                    <section className="space-y-2 max-w-md">
                        ...
                        <Input
                            placeholder={t("navigation:directMessagesPage.friends.searchPlaceholder")}
                            value={friendsQuery}
            ...
            {tab === "recent" && (
                <div className="space-y-4">
                    <section className="space-y-2 max-w-md">
                        ...
                        <Input
                            placeholder={t("navigation:directMessagesPage.recent.searchPlaceholder")}
                            value={recentQuery}
