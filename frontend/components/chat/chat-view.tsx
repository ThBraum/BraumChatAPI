...
import type { Channel, Message, PresenceUser, Thread, UserOnlineStatus } from "@/lib/types";
...
export const ChatView = ({ workspaceId, channelId, threadId }: ChatViewProps) => {
    ...
    const threadsQuery = useQuery<Thread[]>({
        ...
        queryFn: () => apiFetch(`/dm/threads?workspace_id=${encodeURIComponent(workspaceId ?? "")}`),
    ...
    const activeThread = useMemo(() => {
        ...
        const fromList = (threadsQuery.data ?? []).find((th) => String(th.id) === String(threadId));
        if (fromList) return fromList;
        const cached = queryClient.getQueryData<Thread[]>(queryKeys.threadsList(workspaceId ?? undefined)) ?? [];
    ...
    const dmOtherParticipant = useMemo(() => {
        ...
        const otherFromThread = participants.find((p) => String(p.id) !== String(currentUserId ?? "")) ?? null;
    ...
    }, [apiFetch, channelId, dmOtherParticipant?.id, setActiveChannelId, setActiveThreadId, threadId, workspaceId]);

    const dmOnlineQuery = useQuery<UserOnlineStatus[]>({
        queryKey: queryKeys.usersOnline(dmOtherParticipant?.id ? String(dmOtherParticipant.id) : ""),
        queryFn: async () => {
    ...
    const handleIncomingTyping = useCallback(
        (userId: string | number, isTyping: boolean) => {
            ...
            if (!isTyping) {
                ...
                setTypingUserIds((prev) => prev.filter((id) => id !== normalizedUserId));
                return;
            }

            setTypingUserIds((prev) => (prev.includes(normalizedUserId) ? prev : [...prev, normalizedUserId]));
    ...
    const handleIncomingPresence = useCallback(
        (userId: string | number, online: boolean) => {
            ...
            queryClient.setQueryData<UserOnlineStatus[]>(queryKeys.usersOnline(normalizedUserId), [
        ...
        onMessage: (payload) => {
            if (payload.type === "message" && channelId) {
                queryClient.setQueryData<Message[]>(queryKeys.channelMessages(channelId), (prev: Message[] | undefined) => {
                    const list = prev ?? [];
                    const existsById = list.some((msg: Message) => msg.id === payload.payload.id);
                    if (existsById) return list;

                    const incomingClientId = payload.payload.client_id;
                    if (incomingClientId) {
                        const idx = list.findIndex((m) => m.client_id === incomingClientId);
                        if (idx >= 0) {
                            const next = [...list];
    ...
    const sendMessage = useMutation({
        mutationFn: async (values: MessageValues & { client_id: string }) => {
            if (channelId) {
                const sent = channelSocket.send({ type: "message", content: values.content, client_id: values.client_id });
                if (!sent) {
                ...
                const sent = dmSocket.send({ type: "message", content: values.content, client_id: values.client_id });
                if (!sent) {
        ...
        onError: async (_err, values, ctx) => {
            ...
            const stillPresent = list.some(
                (m) => m.client_id === values.client_id || m.id === `client:${values.client_id}`,
            );
            if (stillPresent) return;

            queryClient.setQueryData<Message[]>(targetKey, (prev) =>
                (prev ?? []).filter((m) => m.client_id !== values.client_id && m.id !== `client:${values.client_id}`),
            );
        },
    });
    ...
    const header = useMemo(() => {
        ...
        if (threadId) {
            ...
            const status = dmIsOnline ? t("common:status.online") : t("common:status.offline");
    ...
    useEffect(() => {
        const key = channelId ? `c:${channelId}` : threadId ? `t:${threadId}` : null;
        if (!key) return;
        ...
        <div className="flex h-full">
            <div className="flex flex-1 flex-col px-6 py-4">
                <div className="shrink-0">
                    ...
                    <h2 className="text-lg font-semibold text-foreground">{header.title}</h2>
                ...
                <div className="mt-4 shrink-0">
                        ...
                        <p className="mb-1 text-xs text-muted-foreground">
                            {typingUserIds.length === 1
                                ? t("chat:composer.typing", {
                                    user: splitDiscordDisplayName(resolveDisplayName(typingUserIds[0])).name,
                    ...
                    <MessageComposer
                        ...
                        onSend={async (values) => {
                            await sendMessage.mutateAsync({ ...values, client_id: makeClientId() });
                        }}
