...
def upgrade():
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["workspace_id"],
            ["workspaces.id"],
            name=op.f("fk_direct_message_threads_workspace_id_workspaces"),
        ),
        sa.ForeignKeyConstraint(
            ["user1_id"], ["users.id"], name=op.f("fk_direct_message_threads_user1_id_users")
        ),
        sa.ForeignKeyConstraint(
            ["user2_id"], ["users.id"], name=op.f("fk_direct_message_threads_user2_id_users")
        ),
    ...
    op.create_index(
        op.f("ix_direct_message_threads_id"), "direct_message_threads", ["id"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["thread_id"],
            ["direct_message_threads.id"],
            name=op.f("fk_direct_messages_thread_id_direct_message_threads"),
        ),
        sa.ForeignKeyConstraint(
            ["sender_id"], ["users.id"], name=op.f("fk_direct_messages_sender_id_users")
        ),
